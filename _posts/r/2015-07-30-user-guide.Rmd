...
p <- p %>% plotly_POST(filename="r-user-guide/unemployment")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/unemployment-scatter")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
p <- plot_ly(economics, x = ~date)
p <- add_lines(p, y = ~unemploy / pop, name = "raw")
p <- add_lines(p, y = ~fitted(m), name = "loess")
p
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/unemployment-add-trace")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
p <- economics %>%
  plot_ly(x = ~date) %>%
  add_lines(y = ~unemploy / pop) %>%
  add_lines(y = ~fitted(m)) %>%
  layout(showlegend = F)
p
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/unemployment-fit")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```

Furthermore, `plot_ly()` accepts a data frame as its first argument, and `add_trace()`, and `layout()` accept a plotly object as their first argument. As a result, we can inter-weave data manipulations and visual mappings in a single pipeline.
...
economics %>%
  ...
  layout(
    showlegend = F,
    annotations = list(x = ~date, y = ~unemploy/pop, text = "Peak", showarrow = T)
  )
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/unemployment-layout")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```

`plot_ly()` doesn't require a data frame for input. This makes chart types that accept a `z` argument especially easy to use if you have a numeric matrix:
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/surface")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
```{r, results='hide'}
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/contour")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/qualitative")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```

If you want to change the default palette, it's recommended that you provide a <http://colorbrewer2.org> qualitative palette name (e.g., "Set1" or "Accent") to the colors argument.
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/custom-color")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/set1")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/sequential")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```

In the case of continuous numeric variables, `plot_ly()` performs a linear mapping between the data and an interpolated color palette.
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/linear mapping")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/ggplot2 coloring")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/diverging")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/symbols")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```

To change the default symbols used, use the symbols argument. All the valid symbol types are listed [here](https://plot.ly/r/reference/#scatter-marker-symbol).
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/chaning default symbols")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p <- p %>% plotly_POST(filename="r-user-guide/group")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p3 <- p2 %>% plotly_POST(filename="r-user-guide/group with subplot")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p3$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p3 <- p2 %>% plotly_POST(filename="r-user-guide/subplot range")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p3$url, '.embed"></iframe>', sep='')
```
...
```
```{r, results='asis', echo=FALSE, message=FALSE}
p3 <- p2 %>% plotly_POST(filename="r-user-guide/subplot custom axis")
cat('<iframe style="border: none; width: 100%; height: 550px" src="', p3$url, '.embed"></iframe>', sep='')
```
