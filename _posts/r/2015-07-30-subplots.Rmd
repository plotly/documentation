...
lapply(
  ...
  function(d){
    plot_ly(d, x=~Petal.Length, y=~Petal.Width, name=~Species[[1]]) %>%
      add_markers()
  }
) %>%
  subplot(shareY = TRUE)
...
iris %>% 
  ...
  do(
    p = plot_ly(
      ., x=~Petal.Length, y=~Petal.Width, name=~Species[[1]]
    ) %>%
        add_markers()
  ) %>%
  .[["p"]] %>%
...
`subplot()` can be used to anchor traces onto different axes. In this example, each trace binds itself to one of the 3 x axes with one of these arguments: `xaxis = "x1"`, `xaxis = "x2"`, `xaxis = "x3"`.

```{r, results='hide'}
iris$id <- as.integer(iris$Species)
plot_ly(
  iris, x = ~Petal.Length, y = ~Petal.Width, color = ~Species,
  xaxis = ~paste0("x", id)
) %>%
  add_markers() %>%
  subplot(shareY = TRUE)
...
```{r, results='hide'}
p <- layout(p, title="Overwrite Subplots")
p <- plotly_build(subplot(p))
p$filename <- "overwrite_subplot"
p
```
...
p <- plot_ly(economics, x = ~date) %>%
  add_lines(y = ~uempmed, xaxis = "x1", yaxis = "y1")
p <- add_lines(p, y = ~unemploy, xaxis = "x1", yaxis = "y2")
p <- layout(
  p, showlegend = FALSE,
  yaxis = list(anchor = 'x', domain = c(0, 0.45)),
...
one_plot <- function(dat) {
  dat %>% 
    plot_ly() %>%
    add_trace(
      # setting the geo attribute is _crucial_ here
      lon = ~LON, lat = ~LAT, type = "scattergeo", geo = "geo",
      mode = "markers",
      marker = list(color = toRGB("blue", 0.5))
    ) %>%
    add_trace(
      lon = list(-78), lat = list(47), mode = 'text', text = ~YEAR, type = "scattergeo", geo = "geo"
    ) %>% layout(geo = g)
}
