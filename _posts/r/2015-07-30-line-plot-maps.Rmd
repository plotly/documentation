...
plot_ly(locationmode = 'USA-states', type = 'scattergeo') %>%
  add_trace(
...
plot_ly(lat = ~c(40.7127, 51.5072), lon = ~c(-74.0059, 0.1275)) %>%
  add_trace(type = 'scattergeo', mode = 'lines', line = list(width = 2, color = 'blue')) %>%
  layout(
...
for (i in unique(d$line))
  p <- add_trace(
    p, lat = ~lat, lon = ~lon, data = subset(d, line == i),
    type = 'scattergeo', line = list(width = 2, color = 'violet')
  )
